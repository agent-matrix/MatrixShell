...
def _normalize_url(url: str) -> str:
    ...
    - Add default port if missing for localhost
    """
    url = url.strip()
    ...
    if "localhost" in url or "127.0.0.1" in url:
        ...
        if parsed.port is None:
            # Add default Forge port
            url = f"{parsed.scheme}://{parsed.hostname}:4444"
            if parsed.path:
                url += parsed.path
...
class CatalogConfig:
    ...
    def load(cls, path: Optional[Path] = None) -> "CatalogConfig":
        """Load catalog config with environment variable override.

        Priority (highest wins):
        1. Environment variables (CONTEXT_FORGE_URL, CONTEXT_FORGE_TOKEN, FORGE_URL)
        2. Config file (~/.config/matrixsh/catalog.json)
        3. Defaults
        """
        path = path or _catalog_config_path()
        if not path.exists():
        ...
        env_url = os.environ.get("CONTEXT_FORGE_URL") or os.environ.get("FORGE_URL")
        env_token = os.environ.get("CONTEXT_FORGE_TOKEN")

        if env_url:
            config.url = _normalize_url(env_url)
        if env_token:
            config.token = env_token

        # Auto-enable if both URL and token are set via env
...
class CatalogClient:
    ...
    def login(self, url: str, token: str) -> bool:
        ...
        self.config.url = _normalize_url(url)
