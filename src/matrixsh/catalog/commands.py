...
def cmd_tools(args: argparse.Namespace) -> int:
    """List tools from catalog and synced plugins.

    First tries the catalog API. If no tools found, connects to
    synced plugins to discover tools via MCP protocol.
    """
    client = CatalogClient()
    ...
    # First, try catalog API
    catalog_tools = []
    try:
        catalog_tools = client.list_tools(server_id=args.server)
    except CatalogError:
        pass  # Will try plugin discovery below
    ...
    if catalog_tools:
        ...
        console.print(f"\n[bold]Total:[/bold] {len(catalog_tools)} tool(s) from {len(by_server)} server(s)")
        return 0

    # No tools from catalog API - try discovering from synced plugins
    console.print("[dim]Catalog API returned no tools. Discovering from synced plugins...[/dim]\n")
    ...
    # Filter to catalog-synced plugins only
    catalog_plugins = [p for p in manager.config.plugins if p.source == "catalog" and p.enabled]

    if not catalog_plugins:
        console.print("[yellow]No synced plugins found.[/yellow]")
        console.print("Run [bold]matrixsh sync[/bold] to sync catalog servers.")
        return 0

    # Connect to plugins and discover tools
    total_tools = 0
    table = Table(title="Discovered Tools", show_header=True, header_style="bold cyan")
    table.add_column("Plugin", style="yellow")
    table.add_column("Tool", style="green")
    table.add_column("Description", style="dim")

    for plugin in catalog_plugins:
        console.print(f"[dim]Connecting to {plugin.name}...[/dim]", end=" ")
        ...
        tools = manager.get_plugin_tools(plugin.id)
        if tools:
            for tool in tools:
                desc = tool.description if len(tool.description) <= 50 else tool.description[:47] + "..."
                table.add_row(plugin.name, tool.name, desc)
                total_tools += 1
    ...
    if total_tools > 0:
        console.print(table)
        console.print(f"\n[bold]Total:[/bold] {total_tools} tool(s) from {len(catalog_plugins)} plugin(s)")
    else:
        console.print("[yellow]No tools discovered from plugins.[/yellow]")
        console.print("\nPossible causes:")
        console.print("  - MCP server not running or not reachable")
        console.print("  - Server doesn't expose tools/list endpoint")
        console.print("  - Authentication issue (check your token)")
